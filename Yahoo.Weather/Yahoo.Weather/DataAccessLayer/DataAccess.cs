    ...
    public class DataAccess
    {
        ...
        public int SaveState(Place statesOrCities)
        {
            const string q = "insert into States(Name,woeid)values(@Name,@Woeid) SELECT SCOPE_IDENTITY()";
             _dbhelper.ExecuteQuery(statesOrCities, q);
        ...
        public int IsStateExists(string name,int woeid)
        {
            ...
            return _dbhelper.SelectQuery<int>(new { Name = name, Woeid = woeid }, q).FirstOrDefault(); ;
        }
        public int SaveCity(Place statesOrCities,int stateId)
        {
            ...
            return _dbhelper.ExecuteQuery(new { Name = statesOrCities.Name, Woeid = statesOrCities.Woeid, Stateid = stateId }, q);
        ...
        public int IsCityExists(string name, int woeid, int stateId)
        {
            ...
            return _dbhelper.SelectQuery<int>(new { Name = name, Woeid = woeid, StateId = stateId }, q).FirstOrDefault(); ;
        }
        ...
        public int SaveWeatherInfo(YahooWeatherRssItem yahooWeatherRssItem, int cityId)
        {
            ...
            return _dbhelper.ExecuteQuery(new { Title = yahooWeatherRssItem.Title, Description = yahooWeatherRssItem.Description, Date = yahooWeatherRssItem.Date, Temperature = yahooWeatherRssItem.temp, Cityid = cityId }, q);
